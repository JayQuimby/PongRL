...
import math
import pygame as pg
from game.static import STATE_SPLIT, MAX_SCORE
...
def interpolate_color(value, min_val, max_val, color1, color2):
    ...
    return (
        int(clip(color1[0] * ratio + color2[0] * (1 - ratio), 0, 255)),
        int(clip(color1[1] * ratio + color2[1] * (1 - ratio), 0, 255)),
        int(clip(color1[2] * ratio + color2[2] * (1 - ratio), 0, 255))
    )
...
def timer(func):
    def wrapper(*args, **kwargs):
        start_time = pg.time.get_ticks()    # Start the timer
        result = func(*args, **kwargs)      # Execute the wrapped function
        end_time = pg.time.get_ticks()      # Stop the timer
        runtime = (end_time - start_time) / 1000.0 + 0.0001   # Use the function's name as the message

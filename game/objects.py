...
import random
import math
from game.utils import distance, check_bounds
from game.static import *

ball_delta = lambda x: x * (random.random() + 0.1) * random.choice([-1, 1])
...
class Ball:
    ...
    def reset(self):
        self.x = MID_WIDTH
        self.y = MID_HEIGHT
        self.vx = ball_delta(2.0)
        self.vy = ball_delta(5.0)
    ...
    def move(self):
        ...
        if self.velocity() < 2:
            self.vx /= max(0.1, abs(self.vx))
            self.vy /= max(0.1, abs(self.vy))
        
        if abs(self.vx) < 4:
            self.vx /= max(0.1, abs(self.vx)) / 4
